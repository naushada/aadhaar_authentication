...
uint8_t *ekyc_compose_xml_v25(uint8_t *in_ptr,
  ...
  attr_ptr[3] = uidai_get_attr(param_ptr, "lr");
  attr_ptr[4] = uidai_get_attr(param_ptr, "de");
  attr_ptr[5] = uidai_get_attr(param_ptr, "pfr");
  free(param_ptr);
  param_ptr = NULL;
...
uint8_t *ekyc_main_ex_v20(uint8_t *in_ptr,
  ...
  if(!strncmp(version_ptr, "1.6", 3)) {
    ...
    auth_ptr = auth_main_ex_v20(in_ptr, in_len, len_ptr);

  } else if(!strncmp(version_ptr, "2.5", 3)) {
    auth_ptr = auth_main_ex_v25(in_ptr, in_len, len_ptr);
...
uint8_t *ekyc_main_ex_v21(uint8_t *in_ptr,
  ...
  ekyc_xml_ptr = ekyc_compose_xml_v21(in_ptr, auth_ptr, len_ptr);
  free(auth_ptr);
  auth_ptr = NULL;
...
uint8_t *ekyc_main_ex_v25(uint8_t *in_ptr,
  ...
  ekyc_xml_ptr = ekyc_compose_xml_v25(in_ptr, auth_ptr, len_ptr);
  free(auth_ptr);
  auth_ptr = NULL;
...
uint8_t *ekyc_main_ex(uint8_t *in_ptr, 
  ...
  if(20 == version) {
    ...
    /*ekyc version 2.1*/
    req_ptr = ekyc_main_ex_v21(in_ptr, in_len, &len);

  } else if(25 == version) {
    /*ekyc version 2.5*/
    req_ptr = ekyc_main_ex_v25(in_ptr, in_len, &len);
